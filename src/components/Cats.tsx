...
export default function Cats({ category = null }: { category?: string | null | string[] }) {
    ...
    const getCats = () => {
        axios
            .get(
                'https://api.thedogapi.com/v1/images/search?page=0&limit=30' +
        ...
        <>
            {cats.length > 0 && (
                <ResponsiveMasonry columnsCountBreakPoints={{ 481: 1, 768: 2, 992: 3, 1200: 4 }}>
                    <Masonry>
                        {cats.map((cat: Cat) => (
                            <div key={cat.id} style={{ position: 'relative', height: 400, width: '100%' }}>
                                <Image src={cat.url} alt={cat.id} title={cat.id} style={{ objectFit: 'cover' }} fill unoptimized />
                            </div>
                        ))}
                    </Masonry>
                </ResponsiveMasonry>
